...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        comboBox.pack(side=tk.LEFT, padx=8)
        
        browseButton = ttk.Button(top_row, text="Browse", command=self._pick_image).pack(
            side=tk.LEFT, padx=(8, 0)
        )
        ttk.Button(top_row, text="Run Model", command=self._run_model).pack(
            side=tk.LEFT, padx=(8, 0)
        )
